...
1. 按项目根目录 README 配置 `.env`，执行 `docker compose up -d --build`。本地 `docker-compose.yml` 未传入 `SCHEMA_RUNTIME_MUTATIONS_ENABLED`，应用默认开启幂等建表，`SchemaCompatibilityRunner` 会创建 `market_watch`、`market_snapshot` 和 `market_price_quote`。生产或预发布环境必须保持运行时建表关闭，并在服务启动前按迁移清单执行 `V1.76__market_intelligence.sql`。
